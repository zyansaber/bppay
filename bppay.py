...
OUTPUT_XLSX = ""
GST_RATE = float(os.getenv("GST_RATE", "0.1"))
...
FIREBASE_NODE    = "customer_bp"

# ✅ 不显示的 BP（你要求）
EXCLUDE_BP_EXACT = {
    "0000201371",
    "0000003110", "0000003120", "0000003126",
    "0000003128", "0000003121", "0000003123",
}
EXCLUDE_BP_PREFIXES: Tuple[str, ...] = ()
...
def upload_to_firebase(df: pd.DataFrame) -> None:
    try:
        import firebase_admin
        from firebase_admin import credentials, db
    except Exception as e:
        log.warning(f"firebase_admin not available, skip upload. Reason: {e}")
        return

    if not os.path.exists(FIREBASE_SA_PATH):
        log.warning(f"Firebase service account file not found: {FIREBASE_SA_PATH}. Skip upload.")
        return
    ...
    db.reference(FIREBASE_NODE, app=app).set(payload)
    log.info(f"Uploaded {len(payload['rows'])} rows to Firebase node '{FIREBASE_NODE}'.")
...
def main() -> None:
    ...
    if not df_bp_fin.empty and "BP" in df_bp_fin.columns:
        ...
        agg_cols = ["SIGNED_AMT"]
        first_cols = ["BP", "BP_KEY"]
        df_bp_fin = collapse_by_key(df_bp_fin, "BP_KEY", first_cols=first_cols, sum_cols=agg_cols)
    # 避免merge后出现BP_x/BP_y导致主BP列丢失
    if "BP" in df_bp_fin.columns:
        df_bp_fin = df_bp_fin.drop(columns=["BP"])
    df_bp_fin = ensure_col(df_bp_fin, "BP_KEY")
    df_bp_fin = ensure_col(df_bp_fin, "SIGNED_AMT", 0)
    ...
    # 关键：主报表金额强制与BP_FIN_DETAIL一致（以明细重算SIGNED_AMT）
    if not df_bp_fin_dtl.empty and {"BP", "DRCR", "AMT"}.issubset(df_bp_fin_dtl.columns):
        dtl = df_bp_fin_dtl.copy()
        dtl["BP_KEY"] = dtl["BP"].apply(normalize_bp)
        dtl["AMT"] = pd.to_numeric(dtl["AMT"], errors="coerce").fillna(0)
        dtl["__SIGNED"] = dtl["AMT"]
        df_bp_fin_from_detail = dtl.groupby("BP_KEY", as_index=False)["__SIGNED"].sum().rename(columns={"__SIGNED": "SIGNED_AMT"})
        # 用明细回算金额覆盖汇总金额，避免主表与明细不一致
        if not df_bp_fin.empty:
            df_bp_fin = df_bp_fin.drop(columns=["SIGNED_AMT"], errors="ignore")
        df_bp_fin = df_bp_fin.merge(df_bp_fin_from_detail, on="BP_KEY", how="left")
        df_bp_fin["SIGNED_AMT"] = pd.to_numeric(df_bp_fin["SIGNED_AMT"], errors="coerce").fillna(0)
    ...
    if "BP" in df_bp_name.columns:
        df_bp_name = df_bp_name.drop(columns=["BP"])
    df_bp_name = ensure_col(df_bp_name, "BP_KEY")
    df_bp_name = ensure_col(df_bp_name, "BP_NAME")
    # 避免与左表已有 BP_NAME 冲突导致 pandas merge suffix 报错
    if "BP_NAME" in df_bp_name.columns:
        df_bp_name = df_bp_name.rename(columns={"BP_NAME": "BP_NAME_DIM"})
    ...
    df = (
        ...
        .merge(df_bp_name, on="BP_KEY", how="left")     # BP名称（主BP）
    )

    # 合并后恢复展示主键字段
    if "SO" not in df.columns and "SO_CNT" in df.columns:
        df["SO"] = df["SO_CNT"]
    if "BP_NAME" not in df.columns and "BP_NAME_DIM" in df.columns:
        df["BP_NAME"] = df["BP_NAME_DIM"]
    elif "BP_NAME" in df.columns and "BP_NAME_DIM" in df.columns:
        df["BP_NAME"] = df["BP_NAME"].fillna(df["BP_NAME_DIM"])
    ...
    # 兜底：若历史列名冲突导致BP缺失，回退到BP_KEY
    if "BP" not in df.columns:
        if "BP_x" in df.columns:
            df["BP"] = df["BP_x"]
        elif "BP_y" in df.columns:
            df["BP"] = df["BP_y"]
        elif "BP_KEY" in df.columns:
            df["BP"] = df["BP_KEY"]

    # ✅ 付款方RG 加 name（收钱方名称）
    # 用 KNA1 名称表做映射：BP -> BP_NAME
    bp_name_map = df_bp_name.set_index("BP_KEY")["BP_NAME_DIM"] if (not df_bp_name.empty and "BP_KEY" in df_bp_name.columns and "BP_NAME_DIM" in df_bp_name.columns) else None
    ...
    bp_amount_cols = ["SIGNED_AMT"]
    df = dedupe_bp_amount_display(df, bp_col="BP", amount_cols=bp_amount_cols)

    # Add GST-inclusive net value
    if "SO_NETWR" in df.columns:
        df["SO_NETWR_INCL_GST"] = pd.to_numeric(df["SO_NETWR"], errors="coerce") * (1 + GST_RATE)
    ...
    rename_en = {
        ...
        "SO_NETWR": "Order Net Value",
        "SO_NETWR_INCL_GST": "Order Net Value (Incl. GST)",
        "Currency": "Order Currency",
        "CreatedDate": "Order Created Date",
        "OrderType": "Order Type",
        "CHASSIS_NO": "Chassis Number",
        "SIGNED_AMT": "BP Amount (Signed AMT Sum)",
        "BP_AG": "Sold-to Party (AG)",
        "BP_RE": "Bill-to Party (RE)",
        "BP_RG": "Payer (RG)",
        "BP_RG_NAME": "Payer Name (RG)",
        "BP_WE": "Ship-to Party (WE)",
    }
    df_out = df.rename(columns=rename_en)
    ...
    used_rows = [
        ...
        {"TABLE": "ACDOCA","PURPOSE": "BP金额汇总","FIELDS": "DRCR借方为正、贷方为负后按BP汇总", "CLIENT_COL": bp_fin_meta.get("ACDOCA_client_col")},
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        pd.DataFrame([{
            ...
            "payment_doc_types_for_paid": "N/A",
            ...
            "note": (
                ...
                "已过滤指定BP清单（含0000201371与0000003110/3120/3126/3128/3121/3123）。"
            )
    ...
    # Upload English report rows to Firebase
    upload_to_firebase(df_out)
