...
def sql_bp_financial(conn: pyodbc.Connection, fields: Dict[str, Optional[str]]) -> Tuple[str, List[Any], Dict[str, Optional[str]]]:
    ...
    dedupe_partition = ", ".join([f'a.\"{c}\"' for c in dedupe_cols])
...
def main() -> None:
    ...
    df = (
        df_so_bp
        .merge(df_so_cnt, on="SO", how="left")
        .merge(df_so_chs, on="SO", how="left")      # ✅ chassis
        .merge(df_bp_fin, on="BP", how="left")      # BP汇总金额
        .merge(df_bp_name, on="BP", how="left")     # BP名称（主BP）
    )

    # ✅ 付款方RG 加 name（收钱方名称）
    # 用 KNA1 名称表做映射：BP -> BP_NAME
    bp_name_map = df_bp_name.set_index("BP")["BP_NAME"] if (not df_bp_name.empty and "BP" in df_bp_name.columns and "BP_NAME" in df_bp_name.columns) else None
    if bp_name_map is not None and "BP_RG" in df.columns:
        df["BP_RG_NAME"] = df["BP_RG"].astype(str).map(bp_name_map)
    else:
        df["BP_RG_NAME"] = pd.NA
