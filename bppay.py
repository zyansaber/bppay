...
def sql_bp_financial(conn: pyodbc.Connection, fields: Dict[str, Optional[str]]) -> Tuple[str, List[Any], Dict[str, Optional[str]]]:
    ...
    SELECT
        ...
        SUM(CASE WHEN {credit_cond} THEN -ABS(a."{amt}") ELSE ABS(a."{amt}") END) AS "SIGNED_AMT"
    FROM acdoca_src a
...
def main() -> None:
    ...
    if not df_bp_fin_dtl.empty and {"BP", "DRCR", "AMT"}.issubset(df_bp_fin_dtl.columns):
        ...
        dtl["__SIGNED"] = dtl.apply(
            lambda r: -abs(r["AMT"]) if str(r.get("DRCR", "")).strip() in {"H", "C"} else abs(r["AMT"]),
            axis=1,
        )
